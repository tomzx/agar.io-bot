...
(function(f, g) {
	...
	function findDestination(followMouse) {
		...
		for (var k = 0; k < player.length; k++) {
			var currentPlayer = player[k];
			//console.log("Working on blob: " + k);

			drawCircle(currentPlayer.x, currentPlayer.y, currentPlayer.size + splitDistance, 5);
			...
			allPossibleFood = getAllFood(currentPlayer); // #1

			var allPossibleThreats = getAllThreats(currentPlayer);
			//console.log("Internodes: " + interNodes.length + " Food: " + allPossibleFood.length + " Threats: " + allPossibleThreats.length);
			var allPossibleViruses = getAllViruses(currentPlayer);
			...
			var clusterAllFood = clusterFood(allPossibleFood, currentPlayer.size);

			//console.log("Looking for enemies!");

			for (var i = 0; i < allPossibleThreats.length; i++) {
				var currentThreat = allPossibleThreats[i];
				var enemyDistance = computeDistance(currentThreat.x, currentThreat.y, currentPlayer.x, currentPlayer.y);
				var splitDangerDistance = currentThreat.size + splitDistance + 150;
				var normalDangerDistance = currentThreat.size + 150;
				var shiftDistance = currentPlayer.size;

				//console.log("Found distance.");

				var enemyCanSplit = canSplit(currentPlayer, currentThreat);
				...
				if (enemyCanSplit) {
					drawCircle(currentThreat.x, currentThreat.y, splitDangerDistance, 0);
					drawCircle(currentThreat.x, currentThreat.y, splitDangerDistance + shiftDistance, 6);
				} else {
					drawCircle(currentThreat.x, currentThreat.y, normalDangerDistance, 3);
				...
				if ((enemyCanSplit && enemyDistance < splitDangerDistance) || (enemyCanSplit && currentThreat.danger)) {
					badAngles.push(getAngleRange(currentPlayer, currentThreat, i, splitDangerDistance));
				} else if ((!enemyCanSplit && enemyDistance < normalDangerDistance) || (!enemyCanSplit && currentThreat.danger)) {
					badAngles.push(getAngleRange(currentPlayer, currentThreat, i, normalDangerDistance));
				} else if (enemyCanSplit && enemyDistance < splitDangerDistance + shiftDistance) {
					var tempOb = getAngleRange(currentPlayer, currentThreat, i, splitDangerDistance + shiftDistance);
					var angle1 = tempOb[0];
					var angle2 = rangeToAngle(tempOb);

					obstacleList.push([[angle1, true], [angle2, false]]);
				} else if (!enemyCanSplit && enemyDistance < normalDangerDistance + shiftDistance) {
					var tempOb = getAngleRange(currentPlayer, currentThreat, i, normalDangerDistance + shiftDistance);
					var angle1 = tempOb[0];
					var angle2 = rangeToAngle(tempOb);
			...
			for (var i = 0; i < allPossibleViruses.length; i++) {
				...
				drawCircle(currentVirus.x, currentVirus.y, currentPlayer.size + 50, 3);
				drawCircle(currentVirus.x, currentVirus.y, currentPlayer.size * 2, 6);
			}

			for (var i = 0; i < allPossibleViruses.length; i++) {
				var currentVirus = allPossibleViruses[i];
				var virusDistance = computeDistance(currentVirus.x, currentVirus.y, currentPlayer.x, currentPlayer.y);
				if (virusDistance < (currentPlayer.size * 2)) {
					var tempOb = getAngleRange(currentPlayer, currentVirus, i, currentPlayer.size + 50);
					var angle1 = tempOb[0];
					var angle2 = rangeToAngle(tempOb);
					obstacleList.push([[angle1, true], [angle2, false]]);
				}
			...
			if (badAngles.length > 0) {
				...
				stupidList = addWall(stupidList, currentPlayer);
			}
			...
			for (var i = 0; i < goodAngles.length; i++) {
				var line1 = followAngle(goodAngles[i][0], currentPlayer.x, currentPlayer.y, 100 + currentPlayer.size);
				var line2 = followAngle((goodAngles[i][0] + goodAngles[i][1]).mod(360), currentPlayer.x, currentPlayer.y, 100 + currentPlayer.size);
				drawLine(currentPlayer.x, currentPlayer.y, line1[0], line1[1], 1);
				drawLine(currentPlayer.x, currentPlayer.y, line2[0], line2[1], 1);

				drawArc(line1[0], line1[1], line2[0], line2[1], currentPlayer.x, currentPlayer.y, 1);
			...
			for (var i = 0; i < obstacleAngles.length; i++) {
				var line1 = followAngle(obstacleAngles[i][0], currentPlayer.x, currentPlayer.y, 50 + currentPlayer.size);
				var line2 = followAngle((obstacleAngles[i][0] + obstacleAngles[i][1]).mod(360), currentPlayer.x, currentPlayer.y, 50 + currentPlayer.size);
				drawLine(currentPlayer.x, currentPlayer.y, line1[0], line1[1], 6);
				drawLine(currentPlayer.x, currentPlayer.y, line2[0], line2[1], 6);

				drawArc(line1[0], line1[1], line2[0], line2[1], currentPlayer.x, currentPlayer.y, 6);
			...
			if (followMouse && goodAngles.length === 0) {
				...
				var shiftedAngle = shiftAngle(obstacleAngles, getAngle(tempPoint[0], tempPoint[1], currentPlayer.x, currentPlayer.y), [0, 360]);

				var destination = followAngle(shiftedAngle, currentPlayer.x, currentPlayer.y, distance);

				destinationChoices.push([destination, currentPlayer.size, false]);
